...
<script type="text/javascript">
    ...
    $().ready(function () {
        if ( self !== top ) $('.video-area').addClass('iframe');
...
//                headline: 'My custom "foo" error', // error.js 에서 headline 삭제
        ...
        .then(function(result) {
            if (result.success) {
                ...
                var logUrl = result.data.logUrl;
                var chargeUrl = result.data.chargeUrl;
                var vods = result.data.sources;
                var data = {
                    token: result.data.token,
                    clientIp: result.data.userIp,
                    clientId: getCid(),
                    deviceType: deviceType,
                    osType: osType,
                    uniqueId: result.data.uid,
                    referer: referer,
                    expsId: result.data.expsId
                };
                ...
                player.one('timeupdate', function() {
                    var timer = setInterval(function() {
                        $.get(chargeUrl, $.extend({}, data, {
                            playTime: player.currentTime()
                        })).then(function(result) {
                            if (result.success && result.code === '000000') clearInterval(timer);
                            if (!result.success && result.code === '400001') {
                                clearInterval(timer);
                                errorSend('transmitStop');
                            }
                        });
                    }, 5000);
                });
